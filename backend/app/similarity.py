...
Fallback: spaCy fr_core_news_sm word vectors.
...
def is_in_vocab(word: str) -> bool:
    ...
    if _kv is None:
        return True  # no restriction when model not loaded
...
def embed(word: str) -> np.ndarray | None:
    """Return the unit-norm embedding for a single word, or None if unavailable."""
    return _vec(word)
